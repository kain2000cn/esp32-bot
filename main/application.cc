...
#if TOUCH_ENABLE
    // Schedule([this]() {
    auto &touchx = touch_sensor_manager::Touch::GetInstance();
    touchx.Initialize();
    // });
    xTaskCreatePinnedToCore([](void *arg)
                            {
        Application* app = (Application*)arg;
        app->TouchSensorLoop();
        vTaskDelete(NULL); }, "touch_sensor", 4096, this, 1, &touch_sensor_loop_task_handle_, 1);
...
void Application::TouchSensorLoop()
{
    auto &touchx = touch_sensor_manager::Touch::GetInstance();
    // auto &touchx = touch_sensor_manager::Touch::GetInstance();
    // touchx.Initialize();
...
void Application::CustomPlaySound(const std::string_view &sound, DeviceState *last_state)
{
...
void Application::CustomRecoverSound(DeviceState last_state)
{
    if (GetDeviceState() == kDeviceStateIdle && last_state == kDeviceStateListening)
    {
        auto codec = Board::GetInstance().GetAudioCodec();
        codec->EnableInput(true);
        codec->EnableOutput(false);
        SetDeviceState(last_state);
    }
}
